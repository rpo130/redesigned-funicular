...
def main(args):
    ...
    with torch.no_grad():
        ...
        # img_norm = (old_input_img_squeeze_fill - old_input_img_squeeze_fill.mean(dim=-2, keepdim=True)) / (old_input_img_squeeze_fill.var(dim=-2, unbiased=True, keepdim=True).sqrt() + 1e-6)
        ...
        # rec_img = rec_img * (old_input_img_squeeze_fill.var(dim=-2, unbiased=True, keepdim=True).sqrt() + 1e-6) + old_input_img_squeeze_fill.mean(dim=-2, keepdim=True)
